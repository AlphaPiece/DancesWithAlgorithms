...
/*   Updated: 2018/08/31 16:21:19 by zwang            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char				*flag_collector(t_package *package, char *ptr)
{
	int		i;
	...
	{
		if (ptr[i] == '*')
			asterisk_handler(package, ptr, &i);
		else
...
char				*fmt_handler(t_package *package, char *fmt_ptr)
{
	...
	fmt_spfr = flag_collector(package, fmt_ptr);
	if (!fmt_spfr)
		return (fmt_ptr);
	...
	while (++i < HANDLER_NUM)
		...
		{
			g_handler_table[i].handler(package);
			break ;
		}
	return (fmt_spfr + 1);
}
